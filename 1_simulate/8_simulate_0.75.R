...
for (c in 1:2) {
  ...
  #   save(outcome, file = paste0('outcome/', city_list[c], '_0.75_a.RData'))
  # })
  ...
  system.time({
    outcome <- parLapply(cl, 1:nrow(mcmc_para), run.model, times_lockdown = times_lockdown, p = p0)
    save(outcome, file = paste0('outcome/', city_list[c], '_0.75_c.RData'))
  })
  ...
  system.time({
    outcome <- parLapply(cl, 1:nrow(mcmc_para), run.model, times_lockdown = times_lockdown, p = p0)
    save(outcome, file = paste0('outcome/', city_list[c], '_0.75_b.RData'))
  })
}
